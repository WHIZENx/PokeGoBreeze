...
export const usePVP = () => {
  ...
  const loadPVP = () => {
    APIService.getFetchUrl<APITreeRoot[]>(APIUrl.FETCH_PVP_DATA, getAuthorizationHeaders)
      .then((res) => {
        if (isNotEmpty(res.data)) {
          const pvpTimestamp = new Date(getValueOrDefault(String, res.data[0].commit.committer.date)).getTime();
          if (
            pvpTimestamp !== timestampPVP ||
            !pvpData ||
            ...
            if (pvpUrl) {
              return APIService.getFetchUrl<APITree>(pvpUrl, getAuthorizationHeaders)
                .then((pvpRoot) => {
      ...
      .catch((e: ErrorEvent) => {
        errorProgress({ isError: true, message: `Failed to load PVP data: ${e.message}` });
      });
  };

  const loadPVPMoves = () => {
    APIService.getFetchUrl<PokemonPVPMove[]>(APIUrl.FETCH_PVP_MOVES)
      .then((moves) => {
        dispatch(StoreActions.SetPVPMoves.create(moves.data));
      })
      .catch((e: ErrorEvent) => {
        errorProgress({ isError: true, message: `Failed to load PVP moves: ${e.message}` });
      });
  };
