...
const CardMoveSmall = (props: any) => {
  ...
  const type = props.value
    ? capitalize(combat.find((item: any) => item.name === props.value.name.replace('_FAST', '')).type.toLowerCase())
    : '';
          ...
          {props.value && (
            ...
            >
              ...
              <span style={{ marginRight: 5 }}>
                {splitAndCapitalize(props.value.name.replaceAll('_PLUS', '+').replace('_FAST', ''), '_', ' ')}
              </span>
