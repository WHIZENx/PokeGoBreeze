...
const SelectFind = (props: { data: any[]; clearData: () => void; setStatATK: (arg0: number | null) => void; setStatDEF: (arg0: number | null) => void; setStatSTA: (arg0: number | null) => void; setForm: (arg0: any) => void; setName: (arg0: string) => void; setId: (arg0: null) => void; }) => {
    ...
    const result = useRef(Object.values(pokemonData)
    .filter(pokemon => props.data ? props.data.map((item: { speciesId: any; }) => item.speciesId).includes(convertNameRanking(pokemon.slug)) : true))
    ...
    return (
        <div style={{width: 'fit-content'}}>
            ...
            <div className='card-input' style={{marginBottom: 15}} tabIndex={ 0 } onClick={() => setShowPokemon(true)} onBlur={() => setShowPokemon(false)}>
                <div className='card-select'>
                <CardPokemonLarge id={currentPokemon && currentPokemon.num} name={currentPokemon && splitAndCapitalize(currentPokemon.name, "-", " ") } value={currentPokemon}/>
                {currentPokemon && <button type="button" className='btn-close btn-close-white remove-close' onMouseDown={removePokemon} aria-label="Close"></button>}
                </div>
                {showPokemon &&
                <div className="result-type" onScroll={listenScrollEvent.bind(this)}>
                    <div>
                        {result.current
                        .filter(pokemon => splitAndCapitalize(pokemon.name, "-", " ").toLowerCase().includes(search.toLowerCase()) || pokemon.num.toString().includes(search))
                        .slice(0, firstInit + eachCounter*startIndex).map((pokemon, index) => (
