...
const Evolution = ({forme, region, formDefault, id, onSetIDPoke, gen}: any) => {
    ...
    const getEvoChain = useCallback((data: any) => {
        ...
        if (!evoList) {
            ...
            if (pokemon) return setArrEvoList((oldArr: any) => [...oldArr, [{name: splitAndCapitalize(pokemon.name, "_", " "), id: pokemon.id, baby: false, form: pokemon.form ?? null}]]);
            return setArrEvoList((oldArr: any) => [...oldArr, [{name: (pokemonName as any)[id].name, id: id, baby: false, form: null}]]);
            ...
            if (!evoInJSON && ["Alola", "Galar"].includes(region)) evoInJSON = evoList.find((item: { name: string | string[]; }) => item.name.includes(""))
            if (evoInJSON) {
                const evoInAPI = data.find((item: { species: { url: string; }; }) => parseInt(item.species.url.split("/")[6]) === evoInJSON.id);
                if (evoInJSON.evo_list.length !== evoInAPI.evolves_to.length) {
                    const tempArr: any[] = [];
        ...
        setArrEvoList((oldArr: any) => [...oldArr, data.map((item: { species: { name: any; url: string; }; is_baby: any; form: any; }) => {
    ...
    const getEvoChainJSON = useCallback((id: number, forme: { form_name: string | string[]; name: string; }) => {
        ...
        let pokemon: any = Object.values(pokemonData).find(pokemon => pokemon.num === id && (pokemon.forme ? pokemon.forme.toLowerCase() : pokemon.forme) === form);
        if (!pokemon) pokemon = Object.values(pokemonData).find(pokemon => pokemon.num === id && pokemon.forme === null);
        const prevEvo: any[] = [], curr: any = [], evo: any = [];
    ...
    const getQuestEvo = (prevId: number, form: any) => {
        try {
            return evoData
            .find((item: { evo_list: any[]; }) => item.evo_list
                .find((value: { evo_to_form: string | any[]; evo_to_id: any; }) => value.evo_to_form.includes(form) && value.evo_to_id === prevId))
                .evo_list
                .find((item: { evo_to_form: string | any[]; evo_to_id: any; }) => item.evo_to_form.includes(form) && item.evo_to_id === prevId)
        } catch (error) {
            try {
                return evoData
                .find((item: { evo_list: any[]; }) => item.evo_list
                    .find((value: { evo_to_id: any; }) => value.evo_to_id === prevId))
                    .evo_list
                    .find((item: { evo_to_id: any; }) => item.evo_to_id === prevId)
            } catch (error) {
                return {
                    candyCost: 0,
                    quest: {}
