...
const Form = ({
    ...
    const findForm = () => {
        ...
        return formList.map((form: any[]) => {
            const curFrom = form.find((item: { form: { form_name: any; name: string; }; default_name: string; }) => form && (item.form.form_name === form || item.form.name === item.default_name+"-"+form));
            return curFrom ?? form.find((item: { form: { is_default: any; }; }) => item.form.is_default)
        }).find((item: { form: { form_name: any; name: string; }; default_name: string; id: any; }) => form ? item.form.form_name === form || item.form.name === item.default_name+"-"+form : item.id === id_default);
    }

    const [currForm, setCurrForm] = useState(findForm());
    const [dataPoke, setDataPoke] = useState(pokeData.find((item: { id: any; }) => item.id === id_default));
    ...
    const filterFormList = useCallback((stats: any[], id: any) => {
        ...
        const firstFilter = stats.find((item: { id: any; form: string; }) => item.id === id &&
            currForm.form.form_name.toLowerCase() === item.form.toLowerCase());
        if (firstFilter) return firstFilter;
        const filterForm = stats.find((item: { id: any; form: any; }) => item.id === id &&
    ...
    const changeForm = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
        ...
        const findForm = formList.map((item: any[]) => item.find((item: { form: { name: any; }; }) => item.form.name === name)).find((item: any) => item);
        ...
        if (findData && findForm) {
            ...
            setWH((prevWH: any) => ({...prevWH, weight: pokeData[0].weight, height: pokeData[0].height}));
        }
    ...
    return (
        <Fragment>
            <div className='form-container'>
                ...
                {currForm && pokeID.current &&
                    ...
                    {formList.map((value: any[], index: React.Key | null | undefined) => (
                        <Fragment key={index}>
                            {value.map((value: { form: { name: string; form_name: string; id: null; }; default_name: string; }, index: React.Key | null | undefined) => (
                                <button value={value.form.name+"="+value.form.form_name} key={index} className={"btn btn-form"+(value.form.id === currForm.form.id ? " form-selected" : "")} onClick={(e) => changeForm(e)}>
                                    <img width={64} height={64} onError={(e: any) => {e.onerror=null; e.target.src=APIService.getPokeIconSprite(value.default_name)}} alt="img-icon-form" src={APIService.getPokeIconSprite(value.form.name)}/>
            ...
            <Fragment>
            <hr className="w-100"/>
            {formList.filter((item: { form: { form_name: string | string[]; }; }[]) => item[0].form.form_name.includes("mega")).map((item: { form: any; }[]) => item[0].form).length > 0 && !currForm.form.form_name.includes("gmax") ?
            <div className='row w-100' style={{margin:0}}>
