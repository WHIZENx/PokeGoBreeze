...
const Form = ({
    ...
    const findForm = useCallback(() => {
        ...
        return formList.map((form: any[]) => {
            let curFrom = form.find((item: { form: { form_name: any; name: string; }; default_name: string; }) => form && (item.form.form_name === form || item.form.name === item.default_name+"-"+form));
    ...
    const filterFormList = useCallback((formName: string, stats: any[], id: any, formLength: number) => {
        ...
        else if (filterId.length === formLength && !filterForm) return stats.find((item: { id: any; form: string; }) => item.id === id && item.form === "Normal");
    ...
    return (
        <Fragment>
            <div className='form-container'>
                <div className='scroll-form'>
                    {formList.map((value: any[], index: React.Key | null | undefined) => (
                        <Fragment key={index}>
                            {value.map((value: { form: { id: null; name: string; form_name: string; }; default_name: string; }, index: React.Key | null | undefined) => (
                                <button key={index} className={"btn btn-form "+((currForm && pokeID === currForm.form.id && value.form.id === currForm.form.id) || (currForm && pokeID !== currForm.form.id && value.form.id === currForm.form.id) ? "form-selected" : "")} onClick={() => changeForm(value.form.name, value.form.form_name)}>
                                    <div className='d-flex w-100 justify-content-center'>
                                        <div style={{width: 64}}>
                                            <img className='pokemon-sprite-medium' onError={(e: any) => {
                                                e.onerror=null;
            ...
            <hr className="w-100"/>
            {formList.filter((item: { form: { form_name: string | string[]; }; }[]) => item[0].form.form_name.includes("mega")).map((item: { form: any; }[]) => item[0].form).length > 0 && currForm && !currForm.form.form_name.includes("gmax") ?
            <div className='row w-100' style={{margin:0}}>
