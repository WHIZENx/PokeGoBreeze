    ...
    useEffect(() => {
        const fetchPokemon = async () => {
            if (pokeList.length === 0) {
                ...
                result = result.map((item: { baseStats: { hp: number; atk: number; def: number; spa: number; spd: number; spe: number; }; forme: string | null; num: any; name: any; types: any; color: string; sprite: string; baseSpecies: any; }) => {
        ...
        const result = dataList.filter((item: any) => {
            const boolFilterType = item.types.map((item: any) => selectTypes.includes(item)).filter((bool: boolean) => bool === true).length === selectTypes.length;
        ...
        const onScroll = (e: { target: { documentElement: { scrollTop: any; offsetHeight: any; }; }; }) => {
            ...
            if (scrollTop+windowHeight >= fullHeight*0.6) {
                tableScrollID.current += 1
                setListOfPokemon((oldArr: any) => [...oldArr, ...dataListFilter.current.slice(tableScrollID.current*10, tableScrollID.current*10+10)])
            }
          };
                            ...
                            <Fragment>
                                ...
                                <StyledTableRow>
                                    <StyledTableCell align="center" component="td" colSpan={7}>
                                        <div><span>No match pokémon found</span></div>
                                    </StyledTableCell>
                                </StyledTableRow>
                                :
                                <Fragment>
