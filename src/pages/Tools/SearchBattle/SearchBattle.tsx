...
const FindBattle = () => {
    ...
    const currEvoChain = useCallback((currId: any, form: string, arr: any): void => {
        ...
        else curr = dataStore.evolution.find((item: { id: any; form: string | any[]; }) => currId.includes(item.id) && item.form.includes(form));
        if (!arr.map((i: { id: any; }) => i.id).includes(curr.id)) arr.push({...curr, form: form});
        return currEvoChain(curr.evo_list.map((i: { evo_to_id: any; }) => i.evo_to_id), form, arr)
    }, [dataStore.evolution]);

    const prevEvoChain = useCallback((obj: { id: any; evo_list: any[]; }, defaultForm: any, arr: any[]): any => {
        ...
        const curr = dataStore.evolution.filter((item: { evo_list: any[]; }) => item.evo_list.find((i: { evo_to_id: any; evo_to_form: any; }) => obj.id === i.evo_to_id && i.evo_to_form === defaultForm));
    ...
    const getEvoChain = useCallback((id: any) => {
        const isForm = form.form.form_name.toUpperCase();
        let curr = dataStore.evolution.filter((item: { evo_list: any[]; }) => item.evo_list.find((i: { evo_to_id: any; evo_to_form: any; }) => id === i.evo_to_id && isForm === i.evo_to_form));
        if (curr.length === 0) {
    ...
    const onSearchStatsPoke = useCallback((e: { preventDefault: () => void; }) => {
        e.preventDefault();
    ...
    const getImageList = (id: any) => {
        ...
        let img = dataStore.assets.find((item: { id: any; }) => item.id === id).image.find((item: { form: string | any[]; }) => item.form.includes(isForm));
            ...
            <Fragment>
                ...
                <div className="text-center">
                    <div>
                        ...
                        {bestInLeague.map((value: { id: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | React.ReactFragment | React.ReactPortal | null | undefined; form: string; name: string; ratio: number; league: string; CP: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | React.ReactFragment | React.ReactPortal | null | undefined; rank: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | React.ReactFragment | React.ReactPortal | null | undefined; }, index: React.Key | null | undefined) => (
