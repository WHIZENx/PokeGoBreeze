...
const RaidBattle = () => {
    ...
    const addCPokeData = (dataList: { pokemon: any; fmove: any; cmove: any; dpsDef: number; dpsAtk: number; tdoAtk: number; tdoDef: number; multiDpsTdo: number; ttkAtk: number; ttkDef: number; attackHpRemain: number; defendHpRemain: number; death: number; shadow: any; purified: any; mShadow: any; elite: { fmove: any; cmove: any; }; }[], movePoke: any[], value: { baseStats: { hp: number; atk: number; def: number; spa: number; spd: number; spe: number; }; forme: string | null; types: any; }, vf: string, shadow: boolean, purified: boolean, felite: any, celite: boolean, specialMove: string | any[] | null, pokemonTarget: any) => {
    ...
    const addFPokeData = (dataList: any, combat: { cinematicMoves: any; shadowMoves: any; purifiedMoves: any; eliteCinematicMoves: any; }, movePoke: any[], pokemon: any, felite: boolean, pokemonTarget: boolean) => {
    ...
    const calculateDPSBatlle = (pokemon: { fmoveTargetPokemon: { name: string; }; cmoveTargetPokemon: { name: any; }; dataTargetPokemon: { baseStats: { hp: number; atk: number; def: number; spa: number; spd: number; spe: number; }; forme: string | null; types: any; }; }, HpRemain: number, timer: number) => {
    ...
    const calculateTrainerBatlle = (trainerBattle: any[]) => {
        ...
        const trainerNoPokemon = trainer.filter((pokemons: { filter: (arg0: (pokemon: any) => boolean) => { (): any; new(): any; length: number; }; }) => pokemons.filter((pokemon: { dataTargetPokemon: any; }) => !pokemon.dataTargetPokemon).length > 0);
        ...
        turn.forEach(group => {
            ...
            dataList.summary.tdoAtk = Math.min(dataList.summary.bossHp, dataList.pokemon.reduce((prev: any, curr: any) => prev + curr.tdoAtk, 0));
                        ...
                        <div className="col-lg-7 stats-boss h-100">
                            ...
                            <h5 className="text-decoration-underline">Pokémon at (Level: 40 - 15/15/15)</h5>
                                <ul className="element-top" style={{listStyleType: 'initial'}}>
                                    {resultRaid.map((result: { pokemon: any[]; summary: { tdoAtk: number; timer: number; bossHp: number; dpsAtk: number; }; }, turn: number) => (
                                        <li style={{marginBottom: 15}} key={turn}>
                                            ...
                                            <div className="w-100" style={{overflowX: 'auto'}}>
                                                <table className="table-info table-round-battle">
                                                    ...
                                                    {result.pokemon.map((data: { trainerId: number; pokemon: { sprite: string; name: string; }; dpsAtk: number; tdoAtk: number; atkHpRemain: number; ttkDef: number; hp: number; }, index: React.Key | null | undefined) => (
                                                        <tr key={index}>
