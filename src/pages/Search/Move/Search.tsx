...
const columns: any = [
    ...
    {
        name: 'Type',
        selector: (row: { type: string; }) => <div className={"type-icon-small "+row.type.toLowerCase()}>{capitalize(row.type.toLowerCase())}</div>,
        sortable: true,
        sortFunction: moveSort
    },
    {
        name: 'Name',
        selector: (row: { id: string; name: string; }) => <Link to={"/moves/"+row.id} target="_blank">{splitAndCapitalize(row.name, "_", " ").replaceAll(" Plus", "+")}</Link>,
        sortable: true,
...
const Search = () => {
    ...
    useEffect(() => {
        setResultFMove(combat.filter((item: { type_move: string; }) => item.type_move === "FAST")
        .filter((move: { name: string; id: { toString: () => string | string[]; }; type: string; }) => (splitAndCapitalize(move.name, "_", " ").replaceAll(" Plus", "+").toLowerCase().includes(fMoveName.toLowerCase()) || move.id.toString().includes(fMoveName)) &&
        (fMoveType === '' || fMoveType === capitalize(move.type.toLowerCase()))))
        setResultCMove(combat.filter((item: { type_move: string; }) => item.type_move === "CHARGE").filter((move: { name: string; id: { toString: () => string | string[]; }; type: string; }) => (splitAndCapitalize(move.name, "_", " ").replaceAll(" Plus", "+").toLowerCase().includes(cMoveName.toLowerCase()) || move.id.toString().includes(cMoveName)) &&
