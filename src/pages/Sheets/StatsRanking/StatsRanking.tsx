...
const StatsRanking = () => {
  ...
  useEffect(() => {
    if (
      ...
      const pokemon = dataStore.pokemons.filter((pokemon) => pokemon.num > 0);
  ...
  }, [pokemonList, dataStore.pokemons, stats]);
        ...
        onRowClicked={(row) => {
          if (select?.id !== row.id) {
            ...
            const pokemon = dataStore.pokemons.find(
              (pokemon) => pokemon.num === row.num && isEqual(row.form, pokemon.form)
            );
            if (pokemon) {
