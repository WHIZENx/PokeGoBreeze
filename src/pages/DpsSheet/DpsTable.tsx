...
const columns: any = [
    ...
    {
        name: 'Pokémon Name',
        selector: (row: { pokemon: { num: any; forme: string; name: string; sprite: string; baseSpecies: string; }; shadow: any; purified: any; }) =>
            ...
            onError={(e: any) => {e.onerror=null; e.target.src=APIService.getPokeIconSprite(row.pokemon.baseSpecies)}}/>
    ...
    {
        name: 'Fast Move',
        selector: (row: { fmove: { id: string; name: string; type: string; }; elite: { fmove: any; }; }) => <Link className="d-flex align-items-center" to={"/moves/"+row.fmove.id} target="_blank" title={`${splitAndCapitalize(row.fmove.name, "_", " ")}`}>
    ...
    {
        name: 'Charged Move',
        selector: (row: { cmove: { id: string; name: string; type: string; }; elite: { cmove: any; }; mShadow: any; purified: any; }) => <Link className="d-flex align-items-center" to={"/moves/"+row.cmove.id} target="_blank" title={`${splitAndCapitalize(row.cmove.name, "_", " ")}`}>
    ...
    {
        name: 'DPS',
        selector: (row: { dps: number; }) => parseFloat(row.dps.toFixed(3)),
        sortable: true,
        minWidth: '80px',
    },
    {
        name: 'TDO',
        selector: (row: { tdo: number; }) => parseFloat(row.tdo.toFixed(3)),
        sortable: true,
        minWidth: '100px',
    },
    {
        name: 'DPS^3*TDO',
        selector: (row: { multiDpsTdo: number; }) => parseFloat(row.multiDpsTdo.toFixed(3)),
        sortable: true,
        minWidth: '140px',
    },
    {
...
const DpsTable = () => {
    ...
    const {showEliteMove, showShadow, enableShadow, showMega, enableElite, enableMega, enableBest, enableDelay, releasedGO, bestOf, IV_ATK, IV_DEF, IV_HP, POKEMON_LEVEL} = filters;
    ...
    const addCPokeData = useCallback((dataList: { pokemon: any; fmove: any; cmove: any; dps: number; tdo: number; multiDpsTdo: number; shadow: any; purified: any; mShadow: any; elite: { fmove: any; cmove: any; }; cp: number; }[], movePoke: any, pokemon: any, vf: string, shadow: any, purified: any, felite: any, celite: any, specialMove: any = null) => {
    ...
    const addFPokeData = useCallback((dataList: any, combat: { cinematicMoves: any; shadowMoves: string | any[]; eliteCinematicMoves: any; purifiedMoves: any; }, movePoke: any[], pokemon: any, felite: any) => {
    ...
    const filterBestOptions = (result: any[], best: string | number) => {
        ...
        return Object.values(group).map((pokemon: any) => pokemon.reduce((p: { [x: string]: number; }, c: { [x: string]: number; }) => p[best] > c[best] ? p : c));
    };
    ...
    useEffect(() => {
        ...
        if (finished) {
            let result = dpsTable.filter((item: { fmove: { type: string; }; cmove: { type: string; }; pokemon: { name: string; num: { toString: () => string | any[]; }; forme: string; releasedGO: any; }; shadow: any; elite: { fmove: any; cmove: any; }; }) => {
        ...
        dataTargetPokemon, fmoveTargetPokemon, cmoveTargetPokemon, releasedGO]);
                            ...
                            <div className="row w-100" style={{margin:0}}>
                                <Box className="col-xxl-8" style={{padding:0}}>
                                    <div className="input-group">
                                        ...
                                        <Form.Select style={{borderRadius: 0}} className="form-control" value={bestOf} disabled={!enableBest}
                                ...
                                <Box className="col-xxl-4">
                                    <div className="input-group">
                                        <FormControlLabel control={<Switch checked={releasedGO} onChange={(event, check) => setFilters({...filters, releasedGO: check})}/>} label="Released in GO"/>
                                    </div>
                                </Box>
                            </div>
                        </div>
