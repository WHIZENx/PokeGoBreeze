...
const Pokemon = (props: { id?: any; onDecId?: any; onIncId?: any; isSearch?: any; onSetIDPoke?: any; }) => {
    ...
    const fetchMap = useCallback(async (data: { varieties: any[]; name: string; id: number; generation: { url: string; }; }, axios: { getFetchUrl: (arg0: any, arg1: { cancelToken: any; }) => any; }, source: { token: any; }) => {
        ...
        await Promise.all(data.varieties.map(async (value: { pokemon: { url: any; }; }) => {
            ...
            const poke_form = await Promise.all(poke_info.forms.map(async (item: { url: any; }) => (await axios.getFetchUrl(item.url, {cancelToken: source.token})).data));
        ...
        let defaultFrom, isDefaultForm: { form_name: string; form: { name: string; version_group: { name: string; }; }; default_name: string; name: any; }, defaultData: { weight: any; height: any; };
        let form: any = searchParams.get("form");

        if (form) {
            if (data.id === 555 && form === "galar") form += "-standard"
            defaultFrom = dataFromList.find(value => value.find((item: { form: { form_name: string; name: string; }; default_name: string; }) => item.form.form_name === form.toLowerCase() || item.form.name === item.default_name+"-"+form.toLowerCase()));
    ...
    const queryPokemon = useCallback((id: any, axios: any, source: { token: any; cancel: () => void; }) => {
            ...
            {data &&
                ...
                <div className="w-100 row prev-next-block sticky-top">
                    ...
                    {data.id > 1 && <div title="Previous Pokémon" className={`prev-block col${data.id < maxPokemon-1 ? "-6" : ""}`} style={{padding:0}}>
                        <Link onClick={() => setReForm(false)} className="d-flex justify-content-start align-items-center" to={"/pokemon/"+(data.id-1)} title={`#${data.id-1} ${splitAndCapitalize((pokeListName as any)[data.id-1].name, "-", " ")}`}>
                            ...
                            <div className="w-100" style={{cursor: "pointer"}}>
                                ...
                                <div className="text-navigate">
                                    {splitAndCapitalize((pokeListName as any)[data.id-1].name, "-", " ")}
                                </div>
                            </div>
                        </Link>
                    </div>}
                    {data.id < maxPokemon && (pokeListName as any)[data.id+1] && <div title="Next Pokémon" className={`next-block col${data.id > 1 ? "-6" : ""}`} style={{float: "right", padding: 0}}>
                        <Link onClick={() => setReForm(false)} className="d-flex justify-content-end align-items-center" to={"/pokemon/"+(data.id+1)} title={`#${data.id+1} ${splitAndCapitalize((pokeListName as any)[data.id+1].name, "-", " ")}`}>
                    ...
                    <Fragment>
                    {data.id > 1 && <div title="Previous Pokémon" className={`prev-block col${data.id < maxPokemon-1 ? "-6" : ""}`} style={{padding:0}}>
                        <div className="d-flex justify-content-start align-items-center" onClick={() => props.onDecId()} title={`#${data.id-1} ${splitAndCapitalize((pokeListName as any)[data.id-1].name, "-", " ")}`}>
                            <div style={{cursor: "pointer"}}>
                                <b><NavigateBeforeIcon fontSize="large"/></b>
                            </div>
                            <div style={{width: 60, cursor: "pointer"}}>
                    ...
                    {data.id < maxPokemon && (pokeListName as any)[data.id+1] && <div title="Next Pokémon" className={`next-block col${data.id > 1 ? "-6" : ""}`} style={{float: "right", padding: 0}}>
                        <div className="d-flex justify-content-end align-items-center" onClick={() => props.onIncId()} title={`#${data.id+1} ${splitAndCapitalize((pokeListName as any)[data.id+1].name, "-", " ")}`}>
                            <div className="w-100" style={{cursor: "pointer", textAlign: "end"}}>
                                <div style={{textAlign: "end"}}>
                                    <b>#{data.id+1}</b>
                                </div>
