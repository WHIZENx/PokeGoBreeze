...
const columns: any = [
    ...
    {
        name: 'Name',
        selector: (row: { num: any; forme: string; sprite: string; baseSpecies: string; name: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | React.ReactFragment | React.ReactPortal | null | undefined; }) => <Link to={`/pokemon/${row.num}${row.forme ? `?form=${row.forme.toLowerCase()}`: ""}`} target="_blank"><img height={48} alt='img-pokemon' style={{marginRight: 10}}
        src={APIService.getPokeIconSprite(row.sprite, true)}
        onError={(e: any) => {e.onerror=null; e.target.src=APIService.getPokeIconSprite(row.baseSpecies)}}/>
        {row.name}</Link>,
        sortable: true,
        minWidth: '250px',
...
const Move = (props: { id?: any; }) => {
    ...
    const queryMove = useCallback((id: any) => {
                        ...
                        <table className="table-info move-table">
                            ...
                            <tbody>
                                ...
                                {move.buffs.length > 0 &&
                                    ...
                                    {move.buffs.map((value: { target: string; power: number; type: string; buffChance: number; }, index: React.Key | null | undefined) => (
                                        <tr key={index}>
