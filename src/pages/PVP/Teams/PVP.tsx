...
const TeamPVP = () => {
    ...
    const mappingPokemonData = useCallback((data: { split: (arg0: string) => [any, any]; }) => {
        ...
        let combatPoke = dataStore.pokemonCombat.filter((item: { id: any; baseSpecies: string; }) => item.id === pokemon.num
    ...
    useEffect(() => {
        ...
        const fetchPokemon = async () => {
            ...
            try {
                ...
                const file = (await axios.getFetchUrl(axios.getTeamFile("analysis", params.serie, cp), {
        ...
        <Fragment>
            ...
            <div className="container pvp-container element-bottom">
                ...
                <div className="ranking-container card-container">
                    ...
                    .filter((pokemon: { name: string; }) => splitAndCapitalize(pokemon.name, "-", " ").toLowerCase().includes(search.toLowerCase()))
                    .sort((a: { [x: string]: number; },b: { [x: string]: number; }) => sorted ? b[sortedBy]-a[sortedBy] : a[sortedBy]-b[sortedBy]).map((value: any, index: React.Key | null | undefined) => (
                ...
                <div className="d-grid ranking-container">
                    ...
                    {rankingData.teams
                    .sort((a: { [x: string]: number; },b: { [x: string]: number; }) => sortedTeam ? b[sortedTeamBy]-a[sortedTeamBy] : a[sortedTeamBy]-b[sortedTeamBy]).map((value: any, index: string) => (
                        <Accordion.Item key={index} eventKey={index}>
                            <Accordion.Header>
                                <div className="d-flex align-items-center w-100 justify-content-between" style={{gap: 15}}>
                                    <div className="d-flex" style={{gap: 15}}>
                                    {value.teamData.map((value: { shadow: any; purified: any; form: string; id: any; name: string; }, index: React.Key | null | undefined) => (
                            ...
                            <Accordion.Body style={{padding: 0}}>
                                <Fragment>
                                    {value.teamData.map((value: { pokemonData: { types: any[]; }; shadow: boolean | undefined; purified: boolean | undefined; speciesId: string; form: string; id: any; name: string; fmove: { name: any; id?: string; type?: any; }; combatPoke: { eliteQuickMoves: string | any[]; eliteCinematicMoves: string | any[]; shadowMoves: string | any[]; purifiedMoves: string | any[]; }; cmovePri: { name: any; id?: string; type?: any; }; cmoveSec: { name: any; id?: string; type?: any; }; }, index: React.Key | null | undefined) => (
