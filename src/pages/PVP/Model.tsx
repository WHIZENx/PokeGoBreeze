...
export const Keys = (assets: any[], pokemonData: any[], data: { matchups: any[]; counters: any[]; }, cp: string | undefined, type: string | undefined) => {

    const renderItemList = (data: { opponent: string; rating: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | React.ReactFragment | React.ReactPortal | null | undefined; }, bgtype: number) => {
...
export const MoveSet = (moves: { fastMoves: any[]; chargedMoves: any[]; }, combatList: { eliteQuickMoves: string | any[]; eliteCinematicMoves: string | any[]; }, combatData: any[]) => {
        ...
        <div className='row' style={{margin: 0}}>
            <div className='col-xl-6' style={{padding: 0, backgroundColor: 'lightgray'}}>
                ...
                <div className='type-rank-list'>
                    {moves.fastMoves.map((move: { uses: number; }) => {
                        if (!move.uses) move.uses = 0;
                        return move;
                    }).sort((a: { uses: number; },b: { uses: number; }) => b.uses-a.uses).map((value: any, index: React.Key | null | undefined) => (
            ...
            <div className='col-xl-6' style={{padding: 0, backgroundColor: 'lightgray'}}>
                ...
                <div className='type-rank-list'>
                    {moves.chargedMoves.map((move: { moveId: string; uses: number; }) => {
                        if (move.moveId === "FUTURE_SIGHT") move.moveId = "FUTURESIGHT";
                        if (move.moveId === "TECHNO_BLAST_DOUSE") move.moveId = "TECHNO_BLAST_WATER";
                        if (!move.uses) move.uses = 0;
                        return move;
                    }).sort((a: { uses: number; },b: { uses: number; }) => b.uses-a.uses).map((value: { moveId: any; uses: any; }, index: React.Key | null | undefined) => (
                        <Fragment key={index}>{findMove(value.moveId, value.uses)}</Fragment>
                    ))}
                </div>
            </div>
