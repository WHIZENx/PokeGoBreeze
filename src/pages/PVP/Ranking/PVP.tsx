    ...
    useEffect(() => {
        ...
        const fetchPokemon = async () => {
            ...
            try {
                ...
                let file = (await axios.getFetchUrl(axios.getRankingFile(params.serie, cp, params.type), {
                ...
                file = file.map((item: { speciesId: string; speciesName: string; moveset: string[]; }) => {
                    ...
                    let combatPoke = dataStore.pokemonCombat.filter((item: { id: number; baseSpecies: string; }) => item.id === pokemon.num
        ...
        <div className="container pvp-container element-bottom">
            ...
            <div className="ranking-container">
                ...
                <Accordion alwaysOpen >
                    {rankingData
                    .filter((pokemon: { speciesId: string; speciesName: string; }) => splitAndCapitalize(convertNameRankingToOri(pokemon.speciesId, pokemon.speciesName), "-", " ").toLowerCase().includes(search.toLowerCase()))
                    .sort((a: { [x: string]: number; },b: { [x: string]: number; }) => sorted ? b[sortedBy.current]-a[sortedBy.current] : a[sortedBy.current]-b[sortedBy.current]).map((value: any, index: string) => (
                        <Fragment key={index}>
