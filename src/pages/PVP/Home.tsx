...
const PVPHome = () => {
    ...
    const renderLeagueLogo = (logo: string | string[], cp: number) => {
        if (!logo || (logo && (logo.includes("GBL_littlecup")) || logo.includes("great_league") || logo.includes("ultra_league") || logo.includes("master_league"))) {
    ...
    return (
        <div className="container element-top element-bottom">
            <div className="d-flex align-items-center justify-content-between">
                ...
                <Form.Select style={{height: "fit-content"}} className="w-25 form-control" value={rank.id}
                    onChange={(e) => setOptions({...options, rank: pvp.rankings.find((item: { id: string; }) => item.id === e.target.value)})}>
                        {pvp.rankings.map((value: { id: string | number | readonly string[] | undefined; name: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | React.ReactFragment | React.ReactPortal | null | undefined; }, index: React.Key | null | undefined) => (
            ...
            <div className="group-selected">
                {rank.cp.map((value: number, index: React.Key | null | undefined) => (
                    <Button key={index} className="btn btn-form" style={{height: 200}} target="_blank" href={`/pvp/rankings/${rank.id}/${value}/overall`}>
                        <img alt='img-league' width={128} height={128} src={renderLeagueLogo(rank.logo, value)}/>
                        <div><b>{renderLeagueName(rank.name, value)}</b></div>
            ...
            <div className="d-flex align-items-center justify-content-between">
                ...
                <Form.Select style={{height: "fit-content"}} className="w-25 form-control" value={team.id}
                    onChange={(e) => setOptions({...options, team: pvp.trains.find((item: { id: string; }) => item.id === e.target.value)})}>
                        {pvp.trains.map((value: { id: string | number | readonly string[] | undefined; name: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | React.ReactFragment | React.ReactPortal | null | undefined; }, index: React.Key | null | undefined) => (
                            <option key={index} value={value.id}>{value.name}</option>
                        ))}
                </Form.Select>
            </div>
            <div className="group-selected">
                {team.cp.map((value: number, index: React.Key | null | undefined) => (
                    <Button key={index} className="btn btn-form" style={{height: 200}} target="_blank" href={`/pvp/teams/${team.id}/${value}`}>
                        ...
                        <div><b>{renderLeagueName(team.name, value)}</b></div>
                        <span className="text-danger">CP below {value}</span>
                    </Button>
                ))}
            </div>
