...
export const convertPVPRankings = (data: string[], leagues: League[]) => {
  return [...new Set(data.map((league) => league.split('/').at(0)))].map((league) => {
    ...
    const result = new LeaguePVPPokemonDataModel();
    result.id = league ?? '';
...
export const convertPVPTrain = (data: string[], leagues: League[]) => {
  return [...new Set(data.map((league) => league.split('/').at(0)))].map((league) => {
    let item;
    if (league !== 'all') {
      item = leagues.find((item) => item.iconUrl?.includes(league ?? ''));
